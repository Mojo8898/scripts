...
Running scan_host_list.sh against the the host list `external_hosts.exe`. This file contains a list of target ip addresses separated by newlines
...
Running htb_recon_setup.sh against the box `busqueda` with the target ip address `10.129.228.217`, specifying the VPN file `~/htb/lab_Mojo098.ovpn`
...
Running oscp_recon_setup for the lab `lab01` with the VPN file `~/oscp/universal.ovpn`
